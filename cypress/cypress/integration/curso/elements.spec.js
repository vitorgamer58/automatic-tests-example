...
describe('Work with elements', () => {
    ...
    it('Combo multiplo', () => {
        const lista = ['natacao', 'Corrida', 'nada']
        ...
        cy.get('[data-testid=dataEsportes]').then($el => {
            expect($el.val()).to.be.deep.equal(lista)
            expect($el.val()).to.have.length(3)
        })

        cy.get('[data-testid=dataEsportes]')
            .invoke('val').should('eql', lista)
    ...
    it('Var', () => {
        let texto
        cy.get('.facilAchar').then(($text) => {
            texto = $text.text()
            console.log(texto)
            //cy.get('#formNome').type(txt)
            cy.get('#formNome').type(texto)
        })
